...
var send_audio =  document.getElementById("send-audio");
...
function talk_main_scrolled(){  
   // alert(`scrollheight=${document.querySelector('.talk-main').scrollHeight}; scrollTop=${document.querySelector('.talk-main').scrollTop}; talk-offsetheight=${talk_main.offsetHeight}`)
    ...
    else if(talk_main.scrollHeight > 1000 && talk_main.scrollTop + talk_main.offsetHeight < talk_main.scrollHeight -500){
...
function refresh_chats(){
  ...
  if (this.readyState == 4 && this.status == 200) {
       if(!dont_update_text) {
           ...
           if(just_sent){send_audio.play();talk_main.scrollTo(0,talk_main.scrollHeight);just_sent = false;}
        }
    }}
}
}
